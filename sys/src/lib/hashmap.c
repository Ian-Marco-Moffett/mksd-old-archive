...
static uint32_t 
hashmap_hash(const void* data, size_t length) 
{
...
static uint8_t 
has_duplicates(hashmap_t* hashmap, const char* key) 
{
...
static void* 
find_obj_in_iter(hashmap_entry_t* entry, const char* key) 
{
  hashmap_iter_t* iter;
...
uint8_t 
hashmap_insert(hashmap_t* hashmap, void* obj, const char* key) {
  size_t hash = 0;
...
void* 
hashmap_read(hashmap_t* hashmap, const char* key) 
{
  if (hashmap->entries.len == 0) 
  {
    return NULL;
